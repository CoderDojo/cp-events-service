...
function searchTickets(args, callback) {
  ...
  function getNumberOfApplications(tickets, done) {
    async.map(
      tickets,
      (ticket, cb) => {
        seneca.act({ role: plugin, cmd: 'searchApplications', query: { ticketId: ticket.id, deleted: 0 } }, (err, { length }) => {
  ...
  function getNumberOfApprovedApplications(tickets, done) {
    async.map(
      tickets,
      (ticket, cb) => {
        seneca.act({ role: plugin, cmd: 'searchApplications', query: { ticketId: ticket.id, deleted: 0, status: 'approved' } }, (err, { length }) => {
          if (err) return cb(err);
