...
function saveEvent (args, callback) {
  ...
  function saveEvent (done) {
    ...
    var newEvent = {
      ...
      chooseReleaseTime: eventInfo.chooseReleaseTime
  ...
  function queueTicketRelease (sessions, done) {
    console.log('chooseReleaseTime',eventInfo.chooseReleaseTime);
    if (eventInfo.ticketsReleased === true) {
      return done(null, sessions);
    } else {
        if(eventInfo.chooseReleaseTime){
          ...
          console.log('mili',millisecondsUntilRelease);
          ...
          if (millisecondsUntilRelease < 0) {
            ...
            //send to que and delay the time
          ...
          sendQueue({cmd: 'enqueue', name: 'ticket-release-kue', msg: _.clone({role: 'cd-events', cmd: 'releaseTickets', eventId: eventSaved.id})});
          return done(null, sessions);
        }
      }
  }
