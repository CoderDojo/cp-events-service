...
function bulkApplyApplications(args, callback) {
  ...
  function validateIsParentOf(isSelf, done) {
    ...
    seneca.act({ role: 'cd-profiles', cmd: 'load_user_profile', userId: requestingUser.id }, err => {
      ...
      const childIds = _.filter(applicationUserIds, applicationUserId => applicationUserId !== requestingUser.id);
      async.every(
        childIds,
        (applicationUserId, everyCb) => {
          seneca.act({
            role  : 'cd-users',
            cmd   : 'is_parent_of',
            user  : requestingUser,
  ...
  function validateIsTicketingAdmin(isSelfOrParent, done) {
    if (isSelfOrParent === true) {
      ...
      seneca.act({
        ...
        const isTicketingAdmin = _.find(userDojo.userPermissions, ({ name }) => name === 'ticketing-admin');
      ...
      (application, cb) => {
        ...
        function ensureApplicationIsUnique(cb) {
          seneca.act({
            ...
            query: { sessionId: application.sessionId, userId: application.userId },
          }, (err, applications) => {
            if (err) return cb(err);
            if (applications.length > 0) {
              const ticketFound = _.find(applications, ({ ticketId, deleted }) => ticketId === application.ticketId && !deleted);
  ...
  function generateEmailContent(applications, done) {
    ...
    function retrieveTicketsData(profiles, event, session, dojo, done) {
      async.map(
        applications,
        ({ ticketId, ticketName, ticketType }, cb) => {
          return cb(null, { ticketId: ticketId, ticketName: ticketName, ticketType: ticketType });
        }, (err, tickets) => {
          if (err) return done(err);
          ...
          tickets = _.uniq(tickets, ({ ticketId }) => ticketId);
          let emailCode;
          let dojoEmailCode;
    ...
    function sendEmails(profiles, event, { name }, dojo, tickets, emailCode, dojoEmailCode, done) {
      let emailSubject;
      let emailIntro;
      let eventDate;
      ...
      async.parallel([
        ...
        function sendApplicantsEmail(sendApplicantsCallback) {
          ...
          async.each(
              profiles,
              ({ userId, email, name }, profileCb) => {
                //get the application related to the current profile
                const currentApplication = _.find(applications, ({ userId }) => userId === userId);
        ...
        function sendParentsEmail(sendParentsCallback) {
          ...
          if (applications[0].status === 'pending') {
            ...
            emailIntro = emailIntro
                .ifPlural(childrensNames.length, 'This is an order confirmation for your children %1s for the below event.')
                .fetch([childrensNames.join(', ')]);
          } else if (applications[0].status === 'cancelled') {
