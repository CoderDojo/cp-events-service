      ...
      async.parallel([
        ...
        function sendApplicantsEmail (sendApplicantsCallback) {
          ...
          async.each(profiles, function (profile, profileCb) {
            ...
            sendQueue({cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: payload}, function(err, res) {
              if (err) {
                sendApplicantsCallback(null, err);
              } else {
                sendApplicantsCallback(null, res);
              }
            })});
          });
        },
        ...
        function sendDojoEmail (sendDojoCallback) {
          ...
          if (eventData.notifyOnApplicant) {
            ...
            sendQueue({cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: dojoPayload}, function (err, res) {
                if (err) {
                  sendDojoCallback(null, err);
                } else {
                  sendDojoCallback(null, res);
                }
              })
            });
          }
        ...
        function sendParentsEmail (sendParentsCallback) {
          ...
          sendQueue({cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: parentPayload}, function (err, res) {
              if (err) {
                sendParentsCallback(null, err);
              } else {
                sendParentsCallback(null, res);
              }
            })
          });
        }],
