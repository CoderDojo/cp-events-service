        ...
        seneca.act({role: plugin, cmd: 'searchApplications', query: {sessionId: application.sessionId, userId: application.userId}}, function (err, applications) {
          ...
          if(applications.length > 0) {
            var ticketFound = _.find(applications, function (applicationFound) {
              return applicationFound.ticketName === application.ticketName && applicationFound.ticketType === application.ticketType;
            });
  ...
  function generateEmailContent(applications, done) {
    ...
    function sendEmail(profiles, event, session, dojo, tickets, emailCode, done) {
      process.nextTick(function () {
        var emailSubject;
        (emailCode === 'ticket-application-received-') ? emailSubject = emailSubjectData.request + ' ' + event.name + ' ' + emailSubjectData.received :
        emailSubject = emailSubjectData.request + ' ' + event.name + ' ' + emailSubjectData.approved;
          ...
          function emailParents(cb) {
            ...
            async.eachSeries(profile.parents, function (parent, cb) {
              if(!_.isObject(parent)) {
                seneca.act({role: 'cd-users', cmd: 'load', id: parent}, function (err, parentUser) {
                  if(err) return cb(err);
                  payload.to = parentUser.email;
                  if(!_.contains(parentsEmailed, payload.to)) {
