...
function saveEvent(args, callback) {
  ...
  function saveSessions(event, done) {
    ...
    function removeDeletedSessions(done) {
      seneca.act(
        { role: plugin, cmd: 'searchSessions', query: { eventId: event.id } },
      ...
      async.map(
        ...
        (session, cb) => {
          ...
          if (event.status === 'cancelled') {
            ...
            seneca.act(
              { role: plugin, cmd: 'saveSession', session: sessionToSave },
              (err, savedSession) => {
            ...
            async.each(
              ...
              (existingTicket, cb) => {
                ...
                if (!ticketFound) {
                  ...
                  return seneca.act(
                    { role: plugin, cmd: 'saveTicket', ticket: existingTicket },
                    cb,
                  );
                }
                return cb();
  ...
  function emailInvitedMembers(sessions, done) {
    ...
    process.nextTick(() => {
      async.each(sessions, ({ id }, cb) => {
        seneca.act({ role: plugin, cmd: 'searchSessions', query: { id } }, (err, sessions) => {
          if (err) return cb(err);
          const session = sessions[0];
          async.each(
            session.tickets,
      ...
      function emailInvitedUser({ name }, ticket, { userId }, cb) {
        function sendEmail(cb) {
          seneca.act(
            { role: 'cd-profiles', cmd: 'list', query: { userId } },
            (err, userProfiles) => {
              if (err) return cb(err);
                  ...
                  async.eachSeries(
                    ...
                    (parent, cb) => {
                      if (!_.isObject(parent)) {
                        seneca.act(
                          { role: 'cd-users', cmd: 'load', id: parent, user },
                          (err, { email }) => {
                            if (err) return cb(err);
                            payload.to = email;
                            if (
