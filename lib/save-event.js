      ...
      return done(null, sessions);
        ...
        if(eventInfo.chooseReleaseTime){
          ...
          if (millisecondsUntilRelease < 0) {
            ...
            sendQueue({cmd: 'enqueue', name: 'cp-events-queue', msg: _.clone({role: 'cd-events', cmd: 'releaseTickets', eventId: eventSaved.id}), params: {
          ...
          sendQueue({cmd: 'enqueue', name: 'cp-events-queue', msg: _.clone({role: 'cd-events', cmd: 'releaseTickets', eventId: eventSaved.id})});
          return done(null, sessions);
        }
      }
  }
