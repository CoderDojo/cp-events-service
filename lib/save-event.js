...
function saveEvent (args, callback) {
  ...
  var kue = seneca.export('queues/kue');
  //console.log(kue);
  //console.log(args);
  /*var redis = require("redis"),
    ...
    //client.set("q:jobs:ticket-release-kue", "string val", );
  ...
  function saveEvent (done) {
    ...
    console.log("saveEvent 2:");
    // console.log("ticketsReleased: T or F");
    // console.log(eventInfo.ticketsReleased);
    // console.log("EVENT INFO..");
    // console.log(eventInfo.releaseDate);
    ...
    var newEvent = {
      ...
      ticketsReleased: eventInfo.ticketsReleased,
      releaseDate: eventInfo.releaseDate,
      chooseReleaseDate: eventInfo.chooseReleaseDate
      // private field, back-end behavior only
      // ticketsReleased: eventInfo.ticketsReleased,
    };
  ...
  //adds tickets to a queue to release at their set date and time
  function queueTicketRelease (sessions, done) {
    //ONLY COME IN HERE IFFFFFF - STATUS = PUBLISH...
    // console.log("Queue Ticket Release: ");
    //if tickets have already been released, move on
      //event date
      var eventDate = moment(new Date(eventInfo.date)).utc();
      //releaseDate
      var dateFromUser = moment(new Date(eventInfo.releaseDate)).utc();
      //event date in ms
      var eventTime = eventDate.valueOf();
      // console.log(eventTime);
      //release time in ms
      var time = dateFromUser.valueOf();
      // console.log(time);
      //release date must be before event date
      var eventDateMinusReleaseDate = eventTime - time;
      // console.log(eventDateMinusReleaseDate);

    if (eventInfo.ticketsReleased === true) {
      // console.log("ticket date passed");
      return done(null, sessions);
    } else {
        if(eventDateMinusReleaseDate < 0){

          return done(null, sessions);

        }else{
            // console.log("ticketsReleased = false");
            // console.log("ticket release date is after event date");
            var date;
            //if a date is provided for ticket release
            if (dateFromUser) {
              //set the ticket release date & time
              date = dateFromUser;
            } else {
              //otherwise set to current date & time
              date = moment();
            }
            //ticket release time in milliseconds since unix epoch
            // console.log("time: ");
            // console.log(time);
            ...
            //time to release is the difference between these
            var millisecondsUntilRelease = time - currentTime;
            // console.log("millisecondsUntilRelease: ");
             console.log(millisecondsUntilRelease);
            // console.log("end of Save Event");
            // //if ticket release time has passed
            if (millisecondsUntilRelease < 0) {
              return done('Error: ticket release date has passed and tickets are not released');
            } else {
              //will need an id for the task so that it can be removed and re-added to the kue if the release date is changed
              sendQueue({cmd: 'enqueue', name: 'ticket-release-kue', msg: _.clone({role: 'cd-events', cmd: 'releaseTickets', eventId: eventSaved.id}), params: {
