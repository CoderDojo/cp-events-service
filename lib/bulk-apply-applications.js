...
function bulkApplyApplications (args, callback) {
  ...
  function generateEmailContent (applications, done) {
      ...
      async.map(applications, function (application, cb) {
        ...
        switch (applications[0].status) {
          ...
          case 'cancelled':
            ...
            dojoEmailCode = 'ticket-application-cancelled-to-dojo-';
            break;
    ...
    function sendEmail (profiles, event, session, dojo, tickets, emailCode, dojoEmailCode, done) {
      process.nextTick(function () {
        var emailSubject, dojoEmailSubject;
