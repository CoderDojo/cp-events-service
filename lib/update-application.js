  ...
  function sendEmail(application, done) {
    ...
    function sendEmail(user, event, sendEmailCb) {
      ...
      if(!user.email) { //Approving attendee-u13, retrieve parent email addresses.
        seneca.act({role: 'cd-profiles', cmd: 'list_query', query: {userId:user.id}}, function (err, userProfiles) {
          if(err) return cb(err);
      ...
      function prepareEmails() {
        ...
        if(event.type === 'recurring') {
          ...
          var code = 'recurring-event-application-approved-' + locality;
          var templates = {};
          ...
          //One-off event
          var locality = args.locality || 'en_US';
          var code = 'one-off-event-application-approved-' + locality;
          var templates = {};
