...
function saveEvent (args, callback) {
  ...
  function saveEvent (done) {
    ...
    const maxTicketTypesExceeded = _.find(eventInfo.sessions, (session) => {
    ...
    const pastDateFound = _.find(eventInfo.dates, (date) => {
  ...
  function saveSessions (event, done) {
    eventSaved = event;
    if (_.isEmpty(eventInfo.sessions)) return setImmediate(() => { return done(null, event); });
    function removeDeletedSessions (done) {
      seneca.act({role: plugin, cmd: 'searchSessions', query: {eventId: event.id}}, (err, existingSessions) => {
        if (err) return done(err);
        async.each(existingSessions, (existingSession, cb) => {
          const sessionFound = _.find(eventInfo.sessions, (session) => {
    ...
    function saveNewSessions (done) {
      async.map(eventInfo.sessions, (session, cb) => {
        ...
        if (event.status === 'cancelled') {
          ...
          seneca.act({role: plugin, cmd: 'saveSession', session: sessionToSave}, (err, savedSession) => {
            if (err) return cb(err);
            async.each(session.tickets, (ticket, cb) => {
  ...
  function removeDeletedTickets (sessions, done) {
    ...
    async.each(sessions, (savedSession, cb) => {
      ...
      seneca.act({role: plugin, cmd: 'searchSessions', query: {id: savedSession.id}}, (err, sessions) => {
        if (err) return cb(err);
        const session = sessions[0];
        async.each(session.tickets, (existingTicket, cb) => {
          const ticketFound = _.find(eventInfo.sessions, (session) => {
            return _.find(session.tickets, (ticket) => {
  ...
  function emailInvitedMembers (sessions, done) {
    if (eventInfo.status !== 'published') return setImmediate(() => { return done(null, eventSaved); });
    if (_.isEmpty(sessions)) return setImmediate(() => { return done(null, eventSaved); });
    process.nextTick(() => {
      async.each(sessions, (session, cb) => {
        seneca.act({role: plugin, cmd: 'searchSessions', query: {id: session.id}}, (err, sessions) => {
          if (err) return cb(err);
          const session = sessions[0];
          async.each(session.tickets, (ticket, cb) => {
            if (ticket.deleted === 1) return cb();
            if (_.isEmpty(ticket.invites)) return cb();
            async.eachSeries(ticket.invites, (invite, cb) => {
      ...
      function emailInvitedUser (session, ticket, invite, cb) {
        function sendEmail (cb) {
          seneca.act({role: 'cd-profiles', cmd: 'list', query: {userId: invite.userId}}, (err, userProfiles) => {
            if (err) return cb(err);
            ...
            seneca.act({role: 'cd-dojos', cmd: 'load', id: eventSaved.dojoId}, (err, dojo) => {
              ...
              seneca.act({role: 'cd-dojos', cmd: 'send_email', payload: payload}, (err) => {
                if (err) return cb(err);
              ...
              function emailParents (cb) {
                ...
                async.eachSeries(profile.parents, (parent, cb) => {
                  if (!_.isObject(parent)) {
                    seneca.act({role: 'cd-users', cmd: 'load', id: parent, user: user}, (err, parentUser) => {
                      if (err) return cb(err);
