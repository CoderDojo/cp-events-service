      ...
      function sendConfirmationEmail(status, application, done) {
        ...
        seneca.act({role: plugin, cmd: 'getEvent', id:application.eventId}, function (err, response) {
          ...
          if(event.dates.length > 1) {
            ...
            var eventStartDate = moment(firstDate.startTime).format('MMMM Do YYYY');
            var eventEndDate = moment(lastDate.startTime).format('MMMM Do YYYY');
            var eventTime = moment(firstDate.startTime).format('HH:mm') + ' - ' + moment(lastDate.endTime).format('HH:mm');
            ...
            var eventDate = moment(_.first(event.dates).startTime).format('MMMM Do YYYY, HH:mm');
            var eventTime = moment(_.first(event.dates).startTime).format('HH:mm') + ' - ' + moment(_.first(event.dates).endTime).format('HH:mm');
            var locality = args.locality || 'en_US';
