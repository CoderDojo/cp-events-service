...
function cancelSession (args, callback) {
  ...
  function updateApplicationsStatus (cancelledSession, done) {
    seneca.act({role: plugin, cmd: 'searchApplications', query: {sessionId: cancelledSession.id}}, (err, applications) => {
      if (err) return done(err);
  ...
  function emailSessionAttendees (done) {
    ...
    process.nextTick(() => {
      applicationsData = _.uniq(applicationsData, (application) => { return application.userId; });
