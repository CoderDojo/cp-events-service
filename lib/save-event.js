...
function saveEvent (args, callback) {
  ...
  function saveEvent (done) {
    ...
    var newEvent = {
      ...
      ticketReleaseUse : eventInfo.ticketReleaseUse
    };
    ...
    if (eventInfo.ticketReleaseUse) {
      var utcOffset = moment().utcOffset();
      newEvent.ticketReleaseDate = moment.utc(eventInfo.ticketReleaseDate).subtract(utcOffset, 'minutes').diff(moment.utc(), 'minutes') < 0;
    }
