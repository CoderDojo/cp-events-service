    ...
    function sendEmail(user, event, sendEmailCb) {
      ...
      function prepareEmails() {
        ...
        if(event.type === 'recurring') {
          ...
          var eventStartDate = moment.utc(_.first(event.dates)).format('MMMM Do YYYY');
          var eventEndDate = moment.utc(_.last(event.dates)).format('MMMM Do YYYY');

          var eventTime = moment(eventStartDate.startTime).format('HH:mm') + ' - ' + moment(eventStartDate.endTime).format('HH:mm');
          ...
          var eventTime = moment(eventDate.startTime).format('HH:mm') + ' - ' + moment(eventDate.endTime).format('HH:mm');
          var content = {
            userName: user.name, 
            eventName: event.name,
            eventDate: eventDate,
