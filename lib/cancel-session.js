...
function cancelSession(args, callback) {
  ...
  function updateSessionStatus() {
    return new Promise((resolve, reject) => {
      ...
      seneca.act({ role: plugin, cmd: 'saveSession', session: sessionData }, (err, session) => {
        if (err) reject(err);
        resolve(session);
      });
    });
  ...
  function emailSessionAttendees() {
    return new Promise(resolve => {
      ...
      if (eventData.type === 'recurring') {
        eventDate = `${moment(firstDate.startTime).format(eventDateFormat)} - ${moment(lastDate.startTime).format(
          eventDateFormat
        )} ${startTime} - ${endTime}`;
      } else {
      ...
      process.nextTick(() => {
        ...
        eachSeries(applicationsData, ({ userId, created }, cb) => {
