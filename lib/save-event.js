...
function saveEvent (args, callback) {
    ...
    function saveNewSessions (done) {
      async.map(eventInfo.sessions, function (session, cb) {
        ...
        if (event.status === 'cancelled') {
          ...
          var sessionToSave = _.clone(_.omit(session, ['tickets']));
  ...
  function removeDeletedTickets(sessions, done) {
    if (_.isEmpty(eventInfo.sessions)) return setImmediate(function () { return done(null, sessions); });
    async.each(eventInfo.sessions, function (session, cb) {
      seneca.act({role: plugin, cmd: 'searchTickets', query: {sessionId: session.id}}, function (err, existingTickets) {
        if (err) return done(err);
        async.each(existingTickets, function (existingTicket, cb) {
          var ticketFound = _.find(session.tickets, function (ticket) {
