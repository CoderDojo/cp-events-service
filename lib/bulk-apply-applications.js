          ...
          process.nextTick(function () {
            ...
            async.eachSeries(profiles, function (profile, cb) {
              ...
              function emailParents (cb) {
                ...
                var parentsEmailed = [];
                //for each parent of the current profile
                async.eachSeries(profile.parents, function (parent, cb) {
                  payload.to = parent.email;
                  //if the parent email is not already in parentsEmailed and isn't the same as the child email
                  if (!_.contains(parentsEmailed, payload.to) && payload.to !== profile.email) {
                    //add it to the array
                    parentsEmailed.push(payload.to);
                    //send the email
                    seneca.act({role: 'kue-queue', cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: payload}, cb)});
                  } else {
                    return cb();
                  }
                }, cb);
