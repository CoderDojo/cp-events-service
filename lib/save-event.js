...
function saveEvent (args, callback) {
  ...
  function loadEvent (done) {
    if (eventInfo.id) {
      seneca.act({role: 'cd-events', cmd: 'getEvent', id: eventInfo.id }, function (err, event) {
  ...
  function saveEvent (done) {
    ...
    // set the first publishing time when it's changed to published, or on first save
    if (eventInfo.status === 'published' && ((eventInfo.id && !existingEvent.publishedAt) || (!eventInfo.id))) {
      newlyPublished = true;
    ...
    process.nextTick(function () {
      ...
      function emailInvitedUser (session, ticket, invite, cb) {
        function sendEmail (cb) {
          seneca.act({role: 'cd-profiles', cmd: 'list', query: {userId: invite.userId}}, function (err, userProfiles) {
            ...
            seneca.act({role: 'cd-dojos', cmd: 'load', id: eventSaved.dojoId}, function (err, dojo) {
              if (err) return cb(err);
              var eventDate;
              var firstDate = _.head(eventSaved.dates);
                ...
                async.eachSeries(profile.parents, function (parent, cb) {
                  if (!_.isObject(parent)) {
                    seneca.act({role: 'cd-users', cmd: 'load', id: parent, user: user}, function (err, parentUser) {
                      ...
                      if (!_.includes(parentsEmailed, payload.to) && payload.to !== profile.email) {
        ...
        function updateInvite (cb) {
          ...
          invites = _.without(invites, _.find(invites, {userId: invite.userId}));
