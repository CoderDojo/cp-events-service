...
function bulkApplyApplications (args, callback) {
  ...
  function validateRequest (done) {
    ...
    seneca.act({role: 'cd-users', cmd: 'is_parent_of', childrenIds: _.filter(applicationUserIds, function (applicationUserId) {
    ...
    function sendEmail (profiles, event, session, dojo, tickets, emailCode, done) {
      process.nextTick(function () {
        ...
        if(applications[0].status != 'pending'){
          emailSubject = emailSubjectData.request + ' ' + event.name + ' ' + emailSubjectData[applications[0].status];
          var iterator = 0;
