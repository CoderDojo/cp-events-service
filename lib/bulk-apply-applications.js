...
function bulkApplyApplications (args, callback) {
      ...
      _.each(applications, function (application, cb) {
        available = _.find(tickets, function (ticket) {
          ...
          if(application.ticketId === ticket.id) {
            var ticketsAvailable = (ticket.quantity - ticket.totalApplications) >= applications.length;
          }
  ...
  function generateEmailContent (applications, done) {
    ...
    if (updateAction === 'disapprove' || updateAction === 'checkin' || updateAction === 'delete' || !available) return done(null, applications);
    async.waterfall([
      retrieveProfiles,
    ...
    function sendEmail (profiles, event, session, dojo, tickets, emailCode, done) {
      process.nextTick(function () {
        ...
        if(applications[0].status != 'pending'){
          emailSubject = emailSubjectData.request + ' ' + event.name + ' ' + emailSubjectData.approved;         (emailCode === 'ticket-application-received-') ? emailSubject = emailSubjectData.request + ' ' + event.name + ' ' + emailSubjectData.received : emailSubject = emailSubjectData.request + ' ' + event.name + ' ' + emailSubjectData.approved;
          var iterator = 0;
    ...
    function sendDojoEmail (profiles, event, session, dojo, tickets, emailCode, done) {
      if(event.ticketApproval){
        process.nextTick(function () {
          var emailSubject;
          (emailCode === 'ticket-application-received-') ? emailSubject = emailSubjectData.requestDojo + ' ' + event.name + ' ' + emailSubjectData.received : emailSubject = emailSubjectData.requestDojo + ' ' + event.name + ' ' + emailSubjectData.approved;
          emailCode = emailCode + 'dojo-'
          var iterator = 0;
          async.eachSeries(profiles, function (profile, cb) {
            var eventDate;
            var firstDate = _.first(event.dates);
