...
function validateSessionInvitation(args, callback) {
  ...
  async.waterfall([loadParents, validateRequest, createApplication], err => {
  ...
  function validateRequest(done) {
    if (user.id !== invitedUserId && !_.contains(validParentUserIds, user.id))
      return done(new Error('Only the invited user can accept this invitation.'));
    seneca.act({ role: plugin, cmd: 'loadTicket', id: ticketId }, (err, ticket) => {
      ...
      const invitedUserFound = _.find(ticket.invites, ({ userId }) => {
  ...
  function createApplication({ sessionId, name, type, id }, done) {
    const application = {
      sessionId   : sessionId,
      ticketName  : name,
      ticketType  : type,
    ...
    async.waterfall([loadSession, loadEvent, saveApplication], done);

    function loadSession(done) {
      seneca.act({ role: plugin, cmd: 'loadSession', id: sessionId }, done);
    }

    function loadEvent({ eventId }, done) {
      seneca.act({ role: plugin, cmd: 'getEvent', id: eventId }, (err, event) => {
        if (err) return done(err);
