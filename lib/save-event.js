...
function saveEvent(args, callback) {
  ...
  function saveEvent(done) {
    ...
    const maxTicketTypesExceeded = _.find(eventInfo.sessions, ({ tickets }) => {
    ...
    function removeDeletedSessions(done) {
      seneca.act({ role: plugin, cmd: 'searchSessions', query: { eventId: event.id } }, (err, existingSessions) => {
        ...
        async.each(
          existingSessions,
          (existingSession, cb) => {
            const sessionFound = _.find(eventInfo.sessions, ({ id }) => {
              return existingSession.id === id;
            });
            // The whole section has been deleted but the event is still up
            if (!sessionFound) {
              return seneca.act({ role: plugin, cmd: 'cancelSession', session: existingSession, locality, user }, cb);
            } else {
              return cb();
            }
          }, done
      ...
      async.map(
        ...
        (session, cb) => {
          ...
          if (event.status === 'cancelled') {
            ...
            seneca.act({ role: plugin, cmd: 'cancelSession', session, locality, user }, cb);
          } else {
            session.eventId = event.id;
  ...
  function emailInvitedMembers(sessions, done) {
    ...
    process.nextTick(() => {
      async.each(sessions, ({ id }, cb) => {
        seneca.act({ role: plugin, cmd: 'searchSessions', query: { id: id } }, (err, sessions) => {
          if (err) return cb(err);
          const session = sessions[0];
          async.each(
            session.tickets,
      ...
      function emailInvitedUser({ name }, ticket, { userId }, cb) {
        function sendEmail(cb) {
          seneca.act({ role: 'cd-profiles', cmd: 'list', query: { userId: userId } }, (err, userProfiles) => {
            if (err) return cb(err);
        ...
        function updateInvite(cb) {
          ...
          invites = _.without(invites, _.findWhere(invites, { userId: userId }));
          invites.push(updatedInvite);
          ticket.invites = invites;
          seneca.act({ role: plugin, cmd: 'saveTicket', ticket }, cb);
        }
        async.series([sendEmail, updateInvite], cb);
      }
    });
    return done(null, eventSaved);
  }
