...
function saveEvent (args, callback) {
  ...
  function queueTicketRelease (sessions, done) {
    ...
    if (eventInfo.ticketsReleased === true) {
      ...
      var date;
      //hardcoded release date & time for now (IST)
      ...
      var currentDate = moment().toDate(); //also (IST) for now
      ...
      if (millisecondsUntilRelease < 0) {
        ...
        sendQueue({cmd: 'enqueue', name: 'ticket-release-kue', msg: _.clone({role: 'cd-events', cmd: 'releaseTickets', eventId: eventInfo.id}), params: {
