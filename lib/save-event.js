      ...
      async.eachSeries(eventInfo.invites, function (user, cb) {
        seneca.act({role: 'cd-profiles', cmd: 'list', query: {userId: user.id}}, function (err, response) {
          ...
          var application = {
            ...
            userId: user.id,
            name: user.name,
            dateOfBirth: (response.length === 1) ? response[0].dob : null,
            status: 'approved',
            emailSubject: eventInfo.emailSubject,
            saveFromForm: true
          };
          var dates = {
            prevDateStamp: eventInfo.date,
            newDateStamp: moment.utc(eventInfo.startTime).add(moment().utcOffset(), 'minutes').toISOString()
          };
          seneca.act({role: plugin, cmd: 'updateApplication', application: application, dates: dates}, cb);
        });
      }, done);
    } else {
      setImmediate(function () {
