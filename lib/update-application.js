    ...
    function sendEmail(user, event, sendEmailCb) {
      ...
      function prepareEmails() {
        ...
        if(event.type === 'recurring') {
          ...
          var firstDateParsedZone = moment.parseZone(_.first(event.dates));
          var eventStartDate = firstDateParsedZone.format('MMMM Do YYYY, HH:mm');

          var endDateParsedZone = moment.parseZone(_.last(event.dates));
          var eventEndDate = endDateParsedZone.format('MMMM Do YYYY, HH:mm');

          //TODO: Translate weekdays.
          var weekdays = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
          var eventDay = weekdays[moment(_.first(event.dates)).weekday()];
          var content = { 
            userName: user.name, 
          ...
          var m = moment.parseZone(_.first(event.dates));
          var eventDate = m.format('MMMM Do YYYY, HH:mm');
          var content = {
            userName: user.name, 
            eventName: event.name,
            eventDate: eventDate,
