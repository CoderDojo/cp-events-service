...
function saveEvent (args, callback) {
  ...
  function queueTicketRelease (sessions, done) {
    ...
    if (eventInfo.ticketsReleased === true) {
      ...
      var date;
      //hardcoded release date & time for now (can be any timezone) (if null then it releases immediately)
      var hardCodedDate = moment(new Date(2017, 4, 26, 12, 15, 0, 0)).utc();
      var hardCodedDate = null;
      ...
      if (millisecondsUntilRelease < 0) {
        return done('Error: ticket release date has passed and tickets are not released', null);
      } else {
